...
const TILT = [
  "rotate(-2.4deg) translateY(12px)",
  "rotate(0deg) translateY(0)",
  "rotate(2.4deg) translateY(12px)",
  "rotate(-1.8deg) translateY(8px)",
];
const SPEEDS = [0.1, 0.16, 0.12, 0.14];
  ...
  const go = useCallback(
    (next) => {
      setStart((s) =>
        Math.max(
          0,
          Math.min(maxStart, typeof next === "function" ? next(s) : next),
        ),
      );
    },
    [maxStart],
    ...
    <section id="work" className="pj">
      ...
      >
        <div
          className="pj-track"
          style={{
            transform: `translateX(calc(${-start} * ((100% - 44px) / 3 + 22px)))`,
          }}
        >
          {projects.map((p, i) => {
      ...
      <div className="pj-controls">
        <span className="pj-count">
          ...
          {String(start + VISIBLE).padStart(2, "0")} /{" "}
